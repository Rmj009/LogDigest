        ...
        try:
            ...
            for j in range(arr_3d.shape[1]):
                for i in range(arr_3d.shape[2]):
                    squared_diff_A = (arr_3d[0][j][i] - int(dut_op_mean_lst[0][j])) ** 2
                    print(f"squared_diff_A: ({arr_3d[0][j][i]} - {dut_op_mean_lst[0][j]} )**2 ")
                    squared_diff_B = (arr_3d[1][j][i] - int(dut_op_mean_lst[1][j])) ** 2
                    print(f"squared_diff_B: ({arr_3d[1][j][i]} - {dut_op_mean_lst[1][j]} )**2 ")
                    squared_diff_C = (arr_3d[2][j][i] - int(dut_op_mean_lst[2][j])) ** 2
            ...
            SS_DUT = [(dut_mean_lst[i] - np.average(dut_mean_lst)) ** 2 * dut_test_times for i in
                      range(arr_3d.shape[1])]
            ...
            p_value1 = 1 - f.cdf(1.1442, df_dut, df_repeat) #0.6524410065803709
            p_value2 = 1 - f.cdf(F2, df_op, df_repeat)
            p_value3 = 1 - f.pdf(F3, df_dut_op, df_repeat)
...
if __name__ == '__main__':
    ...
    # 42., 41., 39.66666667, 40.33333333, 41., 42., 40.66666667, 40.33333333, 38.33333333, 42.]),
    # 42.33333333, 42., 40., 40., 41.33333333,41.66666667, 40.33333333, 41.66666667, 39., 39.33333333]),
    # 44., 42., 41., 41., 42.33333333,43.33333333, 41.66666667, 41.66666667, 40., 40.33333333])]
