...
class Gage:
    ...
    def rawData_handling(self, *args):
        ...
        try:
            ...
            op_mean_lst = [np.mean(arr_3d[0][x]) for x in range(arr_3d.shape[0])]  # shape >>> (3, 10, 3)
            ...
            for i in range(arr_3d.shape[1]):
                for j in range(arr_3d.shape[2]):
                    squared_diff_A = (arr_3d[0][i][j] - int(dut_op_mean_lst[0][j])) ** 2
                    squared_diff_B = (arr_3d[1][i][j] - int(dut_op_mean_lst[1][j])) ** 2
                    squared_diff_C = (arr_3d[2][i][j] - int(dut_op_mean_lst[2][j])) ** 2
            ...
            SS_DUT = [(dut_mean_lst[i] - np.average(dut_mean_lst)) ** 2 * arr_3d.shape[1] for i in
                      range(arr_3d.shape[1])]
            ...
            result = (varComp_Repeatability, varComp_reproducibility, varComp_op, varComp_op_dut, varComp_part_to_part, varComp_total_RageRR)
            ...
            grr = total_RageRR / self.range_spec
            print("GageRR: ", grr)
...
#     # print(DUT_1)
#     grr_instance = Gage(DUT_1)
#     Gage.rawData_handling(DUT_1)
